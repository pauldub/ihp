module IHP.IDE.Postgres (startPostgres, stopPostgres, waitUntilServerIsRunning) where
...
waitUntilServerIsRunning :: (?context :: Context) => IO ()
waitUntilServerIsRunning = do
    let isDebugMode = ?context |> get #isDebugMode
    threadDelay 1000000
